...
module.exports.searchFighter = function(fighterName, eventName, cb) {
  
  // Check if optional eventName field is set
  var callback = cb || eventName;
  
  // Parse the fighter's name of spaces and special characters
  var fighter = util.parseName(fighterName);
  
  // Create new URL object
  var options = new Url('https://www.bestfightodds.com/search?query=', fighter, cookieJar);
...
function matchName(results, fighterName, callback) {
  ...
  for (var i = 0; i < results.length; i++) {
    var rgxp = new RegExp(fighterName.toLowerCase(), "g");
...
function loadFighterPage(fighter, callback) {
  var options = new Url(fighter.url, cookieJar);
...
/*
  ...
  - Be careful passing the cookie jar object into the varructor, although it's by reference it may not come out correct
